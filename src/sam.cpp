...
void sam_image_from_cv(const cv::Mat &mat, sam_image_u8 &img) {
  img.nx = mat.cols;
  img.ny = mat.rows;
  img.data.resize(img.nx * img.ny * 3);
  GGML_ASSERT(mat.isContinuous());
  memcpy(img.data.data(), mat.data, img.nx * img.ny * 3);
}
...
bool sam_model_load(const sam_params &params, sam_model &model) {
  ...
  const size_t ctx_size = [&]() {
    ...
    {
      ctx_size +=
          n_enc_state * n_img_embd * n_img_embd * ggml_type_size(GGML_TYPE_F32);

      ctx_size += n_enc_state * 3 * n_patch_size * n_patch_size *
                  ggml_type_size(GGML_TYPE_F16);
      ctx_size += n_enc_state * ggml_type_size(GGML_TYPE_F32);

      ctx_size +=
          n_enc_state * n_enc_out_chans * 1 * 1 * ggml_type_size(GGML_TYPE_F16);
      ctx_size += n_enc_out_chans * n_enc_out_chans * 3 * 3 *
                  ggml_type_size(GGML_TYPE_F16);
    ...
    // image encoder layers
    {
      ...
      ctx_size += n_enc_layer_global * n_enc_head_dim * (2 * n_img_embd - 1) *
                  ggml_type_size(GGML_TYPE_F16);
      ctx_size += n_enc_layer_global * n_enc_head_dim * (2 * n_img_embd - 1) *
                  ggml_type_size(GGML_TYPE_F16);

      ctx_size += n_enc_layer_local * n_enc_head_dim * (2 * n_window_size - 1) *
                  ggml_type_size(GGML_TYPE_F16);
      ctx_size += n_enc_layer_local * n_enc_head_dim * (2 * n_window_size - 1) *
                  ggml_type_size(GGML_TYPE_F16);

      ctx_size += n_enc_layer * 3 * n_enc_state * n_enc_state *
                  ggml_type_size(GGML_TYPE_F16);
      ctx_size += n_enc_layer * 3 * n_enc_state * ggml_type_size(GGML_TYPE_F32);

      ctx_size += n_enc_layer * n_enc_state * n_enc_state *
                  ggml_type_size(GGML_TYPE_F16);
      ctx_size += n_enc_layer * n_enc_state * ggml_type_size(GGML_TYPE_F32);
      ...
      ctx_size += n_enc_layer * 4 * n_enc_state * n_enc_state *
                  ggml_type_size(GGML_TYPE_F16);
    ...
    // prompt encoder
    {
      ctx_size += n_enc_out_chans *
                  ggml_type_size(GGML_TYPE_F16); // 2*(n_enc_out_chans/2)
    ...
    // mask decoder
    {
      // transformer
      {
        ...
        // self_attn
        ctx_size += tfm_layers_count * qkv_count * n_enc_state * n_enc_state *
                    ggml_type_size(GGML_TYPE_F16);
        ctx_size += tfm_layers_count * qkv_count * n_enc_state *
                    ggml_type_size(GGML_TYPE_F32);
        ctx_size +=
            tfm_layers_count * n_enc_state * ggml_type_size(GGML_TYPE_F32);

        // all norms
        ctx_size += tfm_layers_count * norm_count * n_enc_state *
                    ggml_type_size(GGML_TYPE_F32);
        ctx_size += tfm_layers_count * norm_count * n_enc_state *
                    ggml_type_size(GGML_TYPE_F32);
        ...
        ctx_size += tfm_layers_count * qkv_count * n_enc_state *
                    (n_enc_state / 2) * ggml_type_size(GGML_TYPE_F16);
        ctx_size += tfm_layers_count * qkv_count * (n_enc_state / 2) *
                    ggml_type_size(GGML_TYPE_F32);
        ctx_size +=
            tfm_layers_count * n_enc_state * ggml_type_size(GGML_TYPE_F32);

        // mlp
        ctx_size += tfm_layers_count * 8 * n_enc_out_chans * n_enc_out_chans *
                    ggml_type_size(GGML_TYPE_F16);
        ctx_size += tfm_layers_count * 8 * n_enc_out_chans *
                    ggml_type_size(GGML_TYPE_F32);
        ctx_size += tfm_layers_count * n_enc_out_chans * 8 * n_enc_out_chans *
                    ggml_type_size(GGML_TYPE_F16);
        ctx_size +=
            tfm_layers_count * n_enc_out_chans * ggml_type_size(GGML_TYPE_F32);
        ...
        ctx_size += qkv_count * n_enc_state * (n_enc_state / 2) *
                    ggml_type_size(GGML_TYPE_F16);
        ctx_size +=
            qkv_count * (n_enc_state / 2) * ggml_type_size(GGML_TYPE_F32);
        ctx_size += n_enc_state * ggml_type_size(GGML_TYPE_F32);
        ...
        // output_upscaling
        ctx_size += n_enc_out_chans * n_img_embd * 2 * 2 *
                    ggml_type_size(GGML_TYPE_F16);
        ctx_size += 3 * n_img_embd * ggml_type_size(GGML_TYPE_F32);
        ctx_size += n_enc_out_chans * n_img_embd * (n_img_embd / 2) * 2 * 2 *
                    ggml_type_size(GGML_TYPE_F16);
        ctx_size += (n_img_embd / 2) * ggml_type_size(GGML_TYPE_F32);

        // output_hypernetworks_mlps
        ctx_size += n_hypernet_mpls_count * 2 * n_enc_out_chans *
                    n_enc_out_chans * ggml_type_size(GGML_TYPE_F16);
        ctx_size += n_hypernet_mpls_count * 2 * n_enc_out_chans *
                    ggml_type_size(GGML_TYPE_F32);
        ctx_size += n_hypernet_mpls_count * n_enc_out_chans * (n_img_embd / 2) *
                    ggml_type_size(GGML_TYPE_F16);
        ctx_size += n_hypernet_mpls_count * (n_img_embd / 2) *
                    ggml_type_size(GGML_TYPE_F32);

        // iou_prediction_head
        ctx_size += 2 * n_enc_out_chans * n_enc_out_chans *
                    ggml_type_size(GGML_TYPE_F16);
  ...
  // create the ggml context
  {
    struct ggml_init_params params = {
        .mem_size = ctx_size, .mem_buffer = nullptr, .no_alloc = false};
  ...
  // prepare memory for the weights
  {
    ...
    // mask decoder
    {
      ...
      model.tensors
          ...
          dec.transformer_final_attn_token_to_img.v_b;
      model.tensors["mask_decoder.transformer.final_attn_token_to_image.out_"
                    "proj.weight"] =
          dec.transformer_final_attn_token_to_img.out_w;
      model.tensors
    ...
    while (true) {
      ...
      if (ggml_nelements(tensor) != nelements) {
        fprintf(stderr,
                ...
                "expected %d\n",
                __func__, name.data(), (int)nelements,
      ...
      if (tensor->ne[0] != ne[0] || tensor->ne[1] != ne[1] ||
        ...
        fprintf(stderr,
                ...
                __func__, name.data(), (int)ne[0], (int)ne[1], (int)ne[2],
...
struct ggml_tensor *sam_fill_dense_pe(const sam_model &model,
...
struct ggml_cgraph *sam_encode_image(const sam_model &model, sam_state &state,
  ...
  struct ggml_init_params ggml_params = {
      ...
      .no_alloc = true, // skip allocating as we use ggml_alloc to allocate
  ...
  for (int il = 0; il < n_enc_layer; ++il) {
    ...
    // self-attention
    {
      ...
      struct ggml_tensor *KQ_scaled =
          ggml_scale_inplace(ctx0, KQ, 1.0f / sqrtf(n_enc_head_dim));
...

  {
    ...
    auto *data = (float *)ggml_get_data(inp);
...
struct ggml_tensor *sam_prompt_encode_pe_encoding(const sam_encoder_prompt &enc,
                                                  struct ggml_context *ctx0,
                                                  struct ggml_cgraph *gf,
                                                  struct ggml_tensor *coords) {

  auto *cur =
      ggml_mul_mat(ctx0, ggml_cont(ctx0, ggml_transpose(ctx0, enc.pe)), coords);
  cur = ggml_scale(ctx0, cur, float(2.0 * M_PI));
...
// - masks
//
// TODO: currently just encode a single point or box for simplicity
//
prompt_encoder_result sam_encode_prompt(const sam_model &model,
  ...
  auto *embd_prompt_sparse = [&]() {
    ...
    case SAM_PROMPT_TYPE_POINT: {
      ...
      auto *pt_embd_not = ggml_view_2d(ctx0, pt_embd, pt_embd->ne[0], 1,
                                       pt_embd->nb[1], pt_embd->nb[1]);
      ggml_build_forward_expand(
          gf, ggml_cpy(ctx0, enc.not_a_pt_embd_w, pt_embd_not));
      ...
      auto *pt_embd1 =
          ggml_view_2d(ctx0, pt_embd, pt_embd->ne[0], 1, pt_embd->nb[1], 0);
      ggml_build_forward_expand(
          gf, ggml_add_inplace(ctx0, pt_embd1, enc.pt_embd[1]));
    ...
    case SAM_PROMPT_TYPE_BOX: {
      ...
      auto *corner0 = ggml_view_2d(ctx0, corner_embd, corner_embd->ne[0], 1,
                                   corner_embd->nb[1], 0);
      auto *corner1 = ggml_view_2d(ctx0, corner_embd, corner_embd->ne[0], 1,
                                   corner_embd->nb[1], corner_embd->nb[1]);

      ggml_build_forward_expand(
          gf, ggml_add_inplace(ctx0, corner0, enc.pt_embd[2]));
      ggml_build_forward_expand(
          gf, ggml_add_inplace(ctx0, corner1, enc.pt_embd[3]));
...
struct ggml_tensor *sam_decode_mask_transformer_attn(
    ...
    struct ggml_context *ctx0, const sam_model &model) {
  const auto &hparams = model.hparams;
  ...
  struct ggml_tensor *KQ_scaled =
      ggml_scale_inplace(ctx0, KQ, 1.0F / sqrt(float(Q->ne[0])));
...
std::vector<cv::Mat> sam_get_masks(const sam_hparams &hparams, int nx, int ny,
  ...
  if (state.low_res_masks->ne[2] != state.iou_predictions->ne[0]) {
    printf("Error: number of masks (%d) does not match number of iou "
           "predictions (%d)\n",
  ...
  for (int i = 0; i < ne2; ++i) {
    ...
    cv::Mat low_res_mask(ne0, ne1, CV_32F,
                         (float *)(state.low_res_masks->data) + i * ne0 * ne1);
    // Upsample to 1024x1024
    cv::Mat mask_data_;
    cv::resize(low_res_mask, mask_data_, {n_img_size, n_img_size},
               cv::INTER_LINEAR);
    ...
    const float stability_score =
        float(intersections.load()) / float(unions.load());
    ...
    printf("Mask %d: iou = %f, stability_score = %f, intersections = %d, "
           "unions = %d, bbox (%d, %d), (%d, %d)\n",
           i, iou_data[i], stability_score, intersections.load(), unions.load(),
...
struct ggml_cgraph *sam_build_fast_graph(const sam_model &model,
  ...
  prompt_encoder_result enc_res =
      sam_encode_prompt(model, ctx0, gf, state, prompt);
  ...
  // Transform prompt (point or box)
  {
    ...
    // The point scaling here is greatly simplified but mathematically
    // equivalent.
...
void sam_print_usage(int argc, char **argv, const sam_params &params) {
  ...
  fprintf(stderr,
          "                        mask file name prefix (default: %s)\n",
  ...
  fprintf(stderr,
          "                        epsilon decoder transformer (default: %f)\n",
          params.eps_decoder_transformer);
  fprintf(stderr, "SAM prompt:\n");
  fprintf(stderr, "  -p [x,y], --point-prompt\n");
  fprintf(stderr,
          "                        point to be used as prompt for SAM "
          "(default: %f,%f). Must be in a format FLOAT,FLOAT \n",
          params.prompt.pt.x, params.prompt.pt.y);
  fprintf(stderr, "  -b [x1,y1,x2,y2], --box-prompt\n");
  fprintf(stderr,
          "                        box to be used as prompt for SAM (default: "
          "%f,%f,%f,%f). Must be in a format FLOAT,FLOAT,FLOAT,FLOAT \n",
          params.prompt.box.x1, params.prompt.box.y1, params.prompt.box.x2,
...
public:
  ...
  std::vector<cv::Mat> encode_prompts_and_decode_masks(const sam_prompt &prompt,
    ...
    // Encode prompt and decode mask
    {
      ...
      switch (prompt.prompt_type) {
      case SAM_PROMPT_TYPE_POINT:
        snprintf(buf, 128, "Using point prompt: (%f, %f)\n", prompt.pt.x,
                 prompt.pt.y);
        break;
      case SAM_PROMPT_TYPE_BOX:
        snprintf(buf, 128, "Using box prompt: (%f, %f, %f, %f)\n",
                 prompt.box.x1, prompt.box.y1, prompt.box.x2, prompt.box.y2);
        break;
      }
      printf(buf);
